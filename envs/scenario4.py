...
class Scenario4(gym.Env):
	...
	def reset(self):
		...
		self.collision_point = Point(22, 60)
	...
	def get_adv_control(self):
		...
		if self.adv.y > self.turning_point.y:
			...
			steering = -0.1 if (25 - self.adv.x) * np.tan(self.adv.heading) > -5 and np.mod(self.adv.heading, 2*np.pi) > 3*np.pi/2 else 0.1
	...
	def step(self, action):
		...
		self.reward = 1* (self.ego.y - self.target.y)
		# self.reward +=
